...
### Added
- **Parakeet streaming v2 — incremental Partial emission** ([#901](https://github.com/rust-works/omni-dev/issues/901)): Replaces the v1 `StreamingTranscriber` drains-then-batches stub with the full MLX-fork streaming algorithm — local-window relative-position attention, per-layer `RotatingConformerCache` (KV + conv ring buffers), Welford-style mel running stats across chunks, finalized/draft TDT decoder split with persistent `LstmState`. Ported from `parakeet_mlx::parakeet::StreamingParakeet.add_audio` at `newhoggy/parakeet-mlx@32b8034`. New modules `src/voice/backends/parakeet/cache.rs` (557 LOC, 12 unit tests) and additions to `attention.rs` (`RelPositionMultiHeadLocalAttention` with thread-local scratch-tensor caching), `encoder.rs` (`LocalRelPositionalEncoding`, `FastConformerEncoderLocal`), `conformer_block.rs` (`LocalConformerBlock`), `conv_module.rs` (`ConvolutionModule::forward_with_cache`), `decoder.rs` (`TdtDecoder::decode_greedy_stateful`). The streaming wrapper merges incoming source chunks into 5 s internal chunks to amortise candle's per-encoder-forward overhead — measured **RTF 0.24 on the 5-min Sherlock Holmes fixture** (M1 Max multi-thread, default `context_size = (256, 16)`, `depth = 1`), comfortably under realtime. The 30 s slice runs at RTF 0.33. Both previously-`#[ignore]`-blocked streaming tests (`parakeet_streaming_emits_partials_on_30s_slice` and `parakeet_streaming_final_only_matches_batch`) now pass: the former emits ~6 Partials on the 30 s slice (assertion is `>= 2`), the latter produces a transcript substring-equal to the batch path. Defaults trade right-context (16 frames ≈ 1.3 s lookahead) against the MLX fork's validated `(256, 256)` configuration to keep the encoder's per-chunk re-encode window small enough for non-Metal-kernel candle to clear realtime; the WER impact is tracked via the streaming-vs-batch parity check. No silence-gap `Endpoint` detection yet — emits `Endpoint::StreamEnd` only; cpal-driven async-yield-as-you-go also deferred (the v2 wrapper drains chunks synchronously inside the stream closure, then yields the pre-built event sequence).
- **Parakeet-TDT-0.6B-v2 Candle Backend** ([#898](https://github.com/rust-works/omni-dev/issues/898)): Pure-Rust port of `mlx-community/parakeet-tdt-0.6b-v2` against the `candle 0.10.x` runtime, implementing the GO recommendation from the #871 feasibility spike. Reachable as `--backend parakeet-tdt` (or `OMNI_DEV_VOICE_BACKEND=parakeet-tdt`); `whisper-candle` remains the default. The port lives under `src/voice/backends/parakeet/` and is structured as nine sibling modules: `weights.rs` (mmap'd safetensors opener), `audio.rs` (NeMo-style log-mel front-end — 80 bins, n_fft=512, Hann window, Slaney-normalised filterbank, Welford-based per-feature normalisation with cross-chunk running stats), `attention.rs` (Transformer-XL-style rel-pos multi-head attention with the `rel_shift` trick the spike didn't validate — pinned to upstream MLX behaviour via a hand-traced bit-pattern test), `conv_module.rs` (pointwise → GLU → depthwise → BatchNorm → SiLU → pointwise sandwich), `conformer_block.rs` (the standard FF → MHA → conv → FF residual stack with output LayerNorm), `encoder.rs` (24-layer FastConformer + DwStridingSubsampling + sinusoidal positional encoding, with `PARAKEET_0_6B_V2` hardcoded for `feat_in=80`/`d_model=1024`/`n_heads=8`), `decoder.rs` (TDT predictor with a custom 2-layer LSTM stack that loads MLX-named `Wx`/`Wh`/`bias` weights since candle's `candle_nn::LSTM` expects PyTorch-style names, plus the joiner and greedy decode loop with duration-token logic and the MLX anti-stuck rule), `tokenizer.rs` (thin wrapper around `tokenizers::Tokenizer::from_file` for the 1024-token BPE vocab), and `streaming.rs` (v1 `StreamingTranscriber` impl that drains the `AsyncAudioInput` then runs the batch path — true per-chunk incremental `Partial` emission is documented as a follow-up that requires dropping the `Mutex` discipline in favour of `Arc` clones). Also adds the `StreamingTranscriber` + `AsyncAudioInput` trait surface from #806 (`src/voice/transcriber.rs`) plus the `FileAsyncAudioInput` test harness, the production weight-converter script (`scripts/convert_parakeet_weights.py` — argparse, SHA-256 sidecar, atomic write, expected-tensor-count assertions, structured `PARAKEET-CONVERT:` log prefix), the `PARAKEET_TDT_0_6B_V2` `ModelSpec` (`src/voice/models.rs`), and the `--variant parakeet-tdt-0.6b-v2` arm on `voice install-model` which downloads the MLX safetensors from HuggingFace, shells out to the Python converter, deletes the raw 2.47 GB blob, and writes a CC-BY-4.0 `ATTRIBUTION.txt` to the install dir. Covered by 32 unit tests across the new modules (mel filterbank shape + Slaney normalisation, Welford streaming-equals-batch on a single chunk, `rel_shift` hand-traced bit pattern, GLU split semantics, positional-encoding row alignment, predictor zero-state shape, decoder anti-stuck rule documented constant) and three `#[ignore]`-gated end-to-end tests in `tests/voice_transcribe_parakeet_test.rs` covering AC-3a (batch transcript with content-word match), AC-3b (streaming Final-only matches batch), and AC-3c (Partial-event sequence on a 30 s slice — currently gated because v1 streaming emits no Partials). The end-to-end tests require the converted weights staged via `omni-dev voice install-model --variant parakeet-tdt-0.6b-v2` or via `OMNI_DEV_VOICE_PARAKEET_MODEL=<path>`.
...
[Unreleased]: https://github.com/rust-works/omni-dev/compare/v0.28.0...HEAD
