...
mod tests {
    ...
    async fn run_jira_write_enriches_adf_required_error() {
        ...
        let body = serde_json::json!({
            "errorMessages": [],
            "errors": {
                "customfield_10088": "Operation value must be an Atlassian Document (see the Atlassian Document Format)"
            }
        });
        Mock::given(method("PUT"))
            .and(path("/rest/api/3/issue/PROJ-1"))
        ...
        let msg = err.to_string();
        assert!(msg.contains("Field `customfield_10088`"), "got: {msg}");
        assert!(
    ...
    async fn run_jira_write_textarea_object_passes_through() {
        ...
        Mock::given(method("PUT"))
            .and(path("/rest/api/3/issue/PROJ-1"))
            .and(body_json(serde_json::json!({
                "fields": {"customfield_10088": raw_adf.clone()}
            })))
            .respond_with(ResponseTemplate::new(204))
            .expect(1)
            .mount(&server)
        ...
        let client = mock_client(&server.uri());
        let mut extras = std::collections::BTreeMap::new();
        extras.insert("customfield_10088".to_string(), raw_adf);
        run_jira_write(
            &client,
            &mock_cache(),
            "PROJ-1",
