...
//! required parameter rather than an optional filter. Not consumed by
//! anything in Phase 1 — Phase 2's incremental sync is the intended caller.
...
impl<'a> HistoryApi<'a> {
    ...
    /// the mailbox's retention window (about a week); this surfaces here as
    /// an ordinary `ApiRequestFailed{status: 404, ..}` — a future
    /// incremental-sync caller is expected to catch that specific case and
    /// fall back to a full backfill.
    pub async fn list(
        &self,
