...
pub enum FieldSelection {
    ...
    /// field ID (e.g., `customfield_10088`) or a human name (e.g.,
...
pub struct JiraCustomField {
    /// Field ID (e.g., "customfield_10088"). Stable across renames.
    pub id: String,
...
#[derive(Debug, Clone, Default)]
pub struct EditMeta {
    /// Field metadata keyed by field ID (e.g., `customfield_10088`).
...
mod tests {
    ...
    async fn get_issue_with_fields_named_populates_custom_fields() {
        ...
        let issue_json = serde_json::json!({
            ...
            "names": {
                "customfield_10088": "Acceptance Criteria"
            }
        });

        wiremock::Mock::given(wiremock::matchers::method("GET"))
            .and(wiremock::matchers::path("/rest/api/3/issue/ACME-1"))
        ...
        let issue = client
            .get_issue_with_fields(
                "ACME-1",
                FieldSelection::Named(vec!["customfield_10088".to_string()]),
            )
            .await
            .unwrap();
        ...
        assert_eq!(cf.id, "customfield_10088");
    ...
    async fn get_issue_with_fields_standard_omits_custom_fields() {
        ...
        let issue_json = serde_json::json!({
            ...
            "fields": {
                ...
                "priority": null,
                "labels": [],
                "customfield_10088": {"value": "Unplanned"}
            },
            "names": {
                "customfield_10088": "Planned / Unplanned Work"
            }
        });
        ...
        let client = AtlassianClient::new(&server.uri(), "user@test.com", "token").unwrap();
        let issue = client
            .get_issue_with_fields("ACME-1", FieldSelection::Standard)
            .await
            .unwrap();
    ...
    async fn get_issue_with_fields_all_uses_star_param() {
        ...
        let client = AtlassianClient::new(&server.uri(), "user@test.com", "token").unwrap();
        let issue = client
            .get_issue_with_fields("ACME-1", FieldSelection::All)
            .await
            .unwrap();
    ...
    async fn get_editmeta_parses_field_schema() {
        ...
        wiremock::Mock::given(wiremock::matchers::method("GET"))
            .and(wiremock::matchers::path(
                "/rest/api/3/issue/ACME-1/editmeta",
            ))
        ...
        let client = AtlassianClient::new(&server.uri(), "user@test.com", "token").unwrap();
        let meta = client.get_editmeta("ACME-1").await.unwrap();

        assert_eq!(meta.fields.len(), 2);
        let ac = meta.fields.get("customfield_10088").unwrap();
    ...
    async fn update_issue_with_custom_fields_merges_into_payload() {
        ...
        custom.insert(
            "customfield_10088".to_string(),
            serde_json::json!({"type": "doc", "version": 1, "content": [{"type": "paragraph"}]}),
        );
        let result = client
            .update_issue_with_custom_fields("ACME-1", Some(&adf), Some("New title"), None, &custom)
            .await;
        assert!(result.is_ok());
    }
    ...
    async fn update_issue_with_parent_sends_parent_key() {
        ...
        wiremock::Mock::given(wiremock::matchers::method("PUT"))
            .and(wiremock::matchers::path("/rest/api/3/issue/ACME-2"))
            .and(wiremock::matchers::body_json(serde_json::json!({
                "fields": {
                    "description": {"version": 1, "type": "doc", "content": []},
                    "parent": {"key": "ACME-1"}
    ...
    async fn update_issue_shim_sends_no_custom_fields() {
        ...
        let client = AtlassianClient::new(&server.uri(), "user@test.com", "token").unwrap();
        let adf = ValidatedAdfDocument::empty();
        client.update_issue("ACME-1", &adf, None).await.unwrap();
    }
    ...
    async fn get_fields_success() {
        ...
        assert_eq!(fields[3].id, "customfield_10088");
...
impl AtlassianClient {
    ...
    /// `labels`) and custom fields (`customfield_10088`). Returns an error
