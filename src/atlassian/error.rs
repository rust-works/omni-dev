...
pub enum AtlassianError {
    ...
    /// rich-text content in ADF format (e.g. `customfield_10088`) but the
    ...
    JiraAdfFieldRequired {
        /// Stable JIRA field IDs (e.g. `customfield_10088`) whose error
...
mod tests {
    ...
    fn jira_adf_field_required_display_single_field() {
        let err = AtlassianError::JiraAdfFieldRequired {
            fields: vec!["customfield_10088".to_string()],
            original_message:
    ...
    fn jira_adf_field_required_display_multiple_fields() {
        ...
        let msg = err.to_string();
        assert!(
            msg.contains("Fields `customfield_10088`, `customfield_42000`"),
            "got: {msg}"
        );
