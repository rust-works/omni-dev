...
mod tests {
    ...
    async fn run_read_jira_with_fields_jfm_emits_scalars_and_sections() {
        ...
        assert!(rendered.contains("key: ACME-1"));
        ...
        assert!(rendered.contains("<!-- field: Acceptance Criteria (customfield_10088) -->"));
    ...
    async fn run_read_jira_with_fields_adf_emits_field_map_json() {
        ...
        assert_eq!(json["customfield_10088"]["type"], "doc");
    ...
    async fn run_write_jira_with_resolved_fields_force_applies_payload() {
        ...
        let result =
            run_write_jira_with_resolved_fields("ACME-1", &adf, "New", true, &custom, &client)
                .await;
        assert!(result.is_ok());
    }
    ...
    async fn run_write_jira_with_resolved_fields_empty_title_sends_no_summary() {
        ...
        run_write_jira_with_resolved_fields("ACME-1", &adf, "", true, &custom, &client)
            .await
            .unwrap();
    }
    ...
    fn print_jira_dry_run_with_scalars_and_sections() {
        ...
        let sections = [CustomFieldSection {
            ...
            id: "customfield_10088".to_string(),
            body: "- item".to_string(),
        }];
        let result =
            print_jira_dry_run_with_custom_fields("ACME-1", &adf, "T", &scalars, &sections);
        assert!(result.is_ok());
    }
    ...
    fn print_jira_dry_run_without_extras_still_prints_description() {
        ...
        let result = print_jira_dry_run_with_custom_fields("ACME-1", &adf, "", &scalars, &[]);
        assert!(result.is_ok());
    }
