...
- `close` — closes a worktree's window and, for a **linked** worktree, deletes it
  ...
  open, `removable`/`is_main`, blocking risk kinds) and the phase-2 execute
  (self-close vs. cross-window routing, then the terminal outcome — a pruned
  worktree / closed window at INFO, a wait-timeout or prune failure at WARN). Only
  the path and window key are logged, never a secret.
