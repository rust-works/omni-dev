...
- **Model-family marker** (#1448): when the [sessions service](sessions-service.md)
  attributes one or more agent sessions to a worktree, its row description gets a
  trailing `[hsofg*]`-style marker — **h**aiku, **s**onnet, **o**pus, **f**able,
  **g**pt (Codex's `gpt-*` models, #1908), `*` for anything else — naming which model families are running there, in that fixed
