...
> **Note (2026-09-19, #1778):** the "global" placement described in this
> section was partly reversed. `--ai-backend`, `--model`, `--beta-header`,
> and the other AI backend flags are no longer root-`global = true` — they
> are flattened onto the five AI commands via `AiBackendArgs`
> (`src/cli/ai_backend_args.rs`) and placed *after* the subcommand (e.g.
> `omni-dev git commit message twiddle --ai-backend claude-cli ...`). The
> env-bridge mechanism this section describes is unchanged in spirit:
> `AiBackendArgs::apply()` still forwards each set flag into the same
> environment variables at the start of each command's `execute()`, so the
> precedence chain and the rest of this ADR's reasoning still hold — only
> the clap-level placement (root `global = true` vs. per-command flatten)
> changed. See [CLAUDE.md](../../CLAUDE.md)'s "AI Backend Dispatch" section.
...
bridge is unit-tested in isolation. (Note, 2026-09-19, #1778: this export now happens in
`AiBackendArgs::apply`, called at the start of each AI command's `execute()`.)
...
- Global flags are implemented by mutating process-global environment in
  ...
  serialise on the process env. (Note, #1778: the bridge now lives in
  `AiBackendArgs::apply`; the flags are per-command rather than global.)
