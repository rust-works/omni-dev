...
environment variable or the `--models-yaml <PATH>` CLI flag. When set,
...
> **Note (2026-09-19, #1778):** `--models-yaml` is no longer a root-`global
> = true` flag. It is now flattened (via `AiBackendArgs`) onto the five AI
> commands and additionally attached to `config models show`, placed after
> the subcommand (e.g. `omni-dev config models show --models-yaml <PATH>`).
> The env-var override and the merge/precedence semantics below are
> unaffected.
...
- **`OnceLock` initialisation order matters.** `propagate_global_flags` runs
  ...
  fires. (Note, #1778: `--models-yaml` moved off the root-global bridge onto
  `AiBackendArgs::apply()`, called at the top of each AI command's
  `execute()` — still before that command's own `get_model_registry()` call,
  so this ordering guarantee still holds; see the note above.)
