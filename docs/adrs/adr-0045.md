...
2. **The `--profile` flag**, which beats **`OMNI_DEV_PROFILE`**. The flag is a
   *global* flag propagated by `Cli::propagate_global_flags`, which exports its
   ...
   which would be circular. (Note, 2026-09-19, #1778: the method is now
   `Cli::propagate_profile_flag` and handles only `--profile`, which remains the
   one root-global flag; the others moved to the commands that read them.)
