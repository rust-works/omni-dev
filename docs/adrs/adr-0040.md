...
✅ Accepted (2026-06-23). Amended by [ADR-0055](adr-0055.md) (2026-07-20): the
"no secret persisted" property is relaxed for the worktrees service's optional
`webhook` PR source, which persists a single `0600` read token.
...
- Like the rest of the daemon, the service and CLI are `#[cfg(unix)]`; outside WSL2
  there is nothing on Windows for the companion to talk to, and a native (non-WSL)
  Windows port is tracked with the broader daemon work (#1363).
