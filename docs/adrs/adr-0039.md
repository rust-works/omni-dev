...
Which services the registry holds is a **build-time** allowlist, not a runtime
toggle: [`build_default_registry`](../../src/daemon.rs) hosts every service by
default, but a `daemon run --services a,b,c` flag (or the
`OMNI_DEV_DAEMON_SERVICES` env var) narrows it to a subset, skipping the
construction — and therefore the startup work — of any unselected service
([`selection.rs`](../../src/daemon/selection.rs), #1318). Because launchd/systemd
`exec` a fixed command with a minimal environment, `daemon start`/`restart` bake
the selection into the generated plist/unit as a `--services` argument. The
registry stays append-only, so a *runtime* enable/disable op remains a follow-up.
