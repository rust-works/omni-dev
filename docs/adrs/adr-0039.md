...
| Command | Role | Blocks? |
|---|---|---|
| `omni-dev daemon run` | **Becomes** the daemon: acquires the control socket — the launchd-activated fd when socket-activated, else a self-bound socket (the bind is the single-instance lock) — starts every registered service, serves until `SIGTERM`/`SIGINT` (Ctrl-C off-Unix) or a `shutdown` op. This is what launchd demand-spawns (or a launcher execs). | yes |
| `omni-dev daemon start` | Launcher: on macOS installs + bootstraps a launchd LaunchAgent that **owns** the demand socket (the first connect spawns the daemon); elsewhere spawns `daemon run` detached. Warms with one `ping` and polls until the socket answers. | no |
