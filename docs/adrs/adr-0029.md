...
- **Unknown ADF nodes** — node types not in the snapshot are preserved
  via the `adf-unsupported` escape hatch ([ADR-0020](adr-0020.md)'s
  forward-compatibility mechanism): an unknown **block** node round-trips
  as an ` ```adf-unsupported ` fenced block, and an unknown **inline**
  node as an `:adf-unsupported[]{json="…"}` directive (issue #1117 — before
  which inline unknowns collapsed to a lossy `<!-- unsupported inline -->`
  comment). Both carry the full node JSON, so they survive a round trip but
  do not gain any Markdown-native syntax. [ADR-0023](adr-0023.md)'s
