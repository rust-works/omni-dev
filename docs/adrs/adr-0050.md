...
🔄 Superseded by [ADR-0053](adr-0053.md) (2026-07-15)

Badges are now resolved in the daemon and pushed over the `subscribe` stream.
Resolving them extension-side on repo-expand meant nothing ever re-asked GitHub
once a badge was computed — the streamed snapshot carries worktree topology, not CI
— so a badge went stale the moment CI moved and stayed stale (#1337).

The decision below is retained as a historical record. Note its central argument —
that daemon-side resolution *"would introduce a GitHub token into the daemon"* —
does **not** apply to ADR-0053, which shells out to `gh` and so keeps the credential
out of the daemon exactly as this ADR wanted. What ADR-0053 gives up is narrower:
PR data now crosses the `0600` same-user socket. See ADR-0053.
