#!/usr/bin/env bash
# Codex Stop hook: block stop when CLI surface changed without snapshot
# updates.
#
...
# that gate under Codex by exiting non-zero with a stderr message, which
# Codex surfaces back to the model.
set -u
...
# Stop is re-fired after a continuation. Do not loop indefinitely.
